...
const PROVIDER_ID = '8c425a0d-1682-4fdd-af5d-a4ced8bfd7a7'; // claude 段里 name = deepseek官网 的记录
const PROVIDER_NAME = 'deepseek官网';
...
if (typeof env.ANTHROPIC_AUTH_TOKEN !== 'string' || env.ANTHROPIC_AUTH_TOKEN.length === 0) {
    // 仅空值兜底：从凭据仓取 DEEPSEEK_API_KEY（正常路径不走到这）
    const credRaw = fs.readFileSync(CREDENTIALS, 'utf8');
    const m = /DEEPSEEK_API_KEY\s*:\s*["']?([^"'\s]+)/.exec(credRaw);
    if (!m) throw new Error('模板 token 为空且凭据仓没有 DEEPSEEK_API_KEY，中止');
    changes.ANTHROPIC_AUTH_TOKEN = ['(空)', m[1]];
    log('⚠ token 为空，将从凭据仓补（正常情况 token 一律不动）');
}
...
const preChecks = {
    ...
    '其它供应商逐字节一致': otherProvidersOk,
    '非目标 env 键不变（token/effort 等）': ALIAS_KEYS.every((k) => true) && same(
...
log('回读校验: PASS（去 CCGUI 供应商管理 → Claude 页签 → deepseek官网 → 保存/应用 后才写入 settings.json 生效）');
