...
- **全量收录**：官方「当前支持的模型列表」里的模型**一个不落**，只增不减（官方下架才删）；**不做三线口味精简、不跨渠道去重、不与官网/智谱/百炼合并口径**。**阈值豁免（2026-09-20 定，用户口径）**：请求数低于 1000 的模型（name 里 `5h·` 后的数字 <1000）**不收录、不进任何一条路由**；`5h·无限` 的条目不算低于 1000、照常收录；促销回落或官方调次数导致某条跌破 1000 时，下一轮巡检自动摘掉该条。
...
- **命名（2026-09-25 用户口径改为缩写）**：`[新/][N倍/]OpenCode/<显示名>/<参数量?>(/<版本?>)/5h·N`
  - **标记区前置**：官方「新」徽章拼 `新/`、落地页限时倍数拼 `N倍/`，两者都放在 `OpenCode` **前边**（都有则 `新/4倍/OpenCode/…`；只有一个就只拼那个；都没有则省略）；
  ...
  - 参数量只标 §4 已核实数字，未披露不标；
  - 调用次数以 `5h·N`（无预估写 `5h·无限`）拼在名字末尾——**别再写「每5小时N次」长串**（2026-09-25 用户嫌长）；整表按**显示值**（name 里 `5h·` 后的数字，**促销值优先**，如 V4.1 Flash 用 26000 而非基础 6500）**倒序**；标记区（`新/`、`N倍/`）只随行移动、不参与排序；同次数按官方表原序。
...
3. **取值规则**：带限时促销的模型取**落地页促销值**（26,000），并在 name 前置 `N倍/` 标记；促销结束、落地页回落基础值时，巡检自动改回基础值并去掉该标记；
...
1. `YAML.parse` 通过；
2. **id 集合 diff（两路由并集，按 1000 阈值豁免口径，2026-09-20 定）**：官方列表 −（`opencode-go` ∪ `opencode-go-anthropic`）允许非空，但其中每一条都必须是次数<1000 的模型（凡次数≥1000 或「无限」的条目漏配即判失败）**且** 并集 − 官方 = ∅，且并集内**无重复 id**；
3. 条数比对：官方 N − 其中次数<1000 的条数 = 两条路由条数之和；
