...
4. **官网数据块（徽章与次数真源，覆盖全部 32 条）**：落地页 HTML 源码里找 `/_build/assets/index-*.js` 脚本地址（可能多个，逐个抓，内含全部模型数组——每条 `id/name/requests/allowance`，可带 `featured/fresh/limitedTime/regions`——的那个就是）；字段口径：`fresh`=「新」徽章、`limitedTime`=末尾 `/限`、`requests`=次数与 1000 阈值依据；**非 featured 模型的徽章只有数据块里有**（首屏 HTML 看不见，实例：MiMo-V2.6-Pro `fresh:true` 但不在首屏）；数据块与①②数值冲突时先复核再取舍并写进报告；
